...
#   new_vec = c( rep(1, nCount[i]), rep(0, 2000-nCount[i])  )
#   counts = c(counts, new_vec)
# }
# 
# count_matrix = matrix(counts, nrow=2000, ncol=length(counts)/2000   ,
#                       dimnames = list(c(paste0("row", as.character(seq(2000))  ) ),
#                                                         paste0("C",   as.character(seq(length(counts)/2000 )) ) )
#                       )

out = fit_3_normals(nCount)
out_atac = fit_3_normals_atac(round(atac_downsampling*nCount))

test_that("lower is bigger than barhop_end", {
